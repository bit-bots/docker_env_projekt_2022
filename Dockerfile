...
# Basic Utilities
RUN apt-get -y update && apt-get install -y eatmydata && eatmydata apt-get -y upgrade && eatmydata apt-get install -y zsh screen tmux tree sudo ssh synaptic htop vim tig ipython ipython3 less ranger gdb iproute2 iputils-ping vlc beignet wget gnupg2
...
RUN apt-get install -y libncurses5-dev uvcdynctrl python3-yaml python3-opencv python3-numpy python3-catkin-lint
